...
void TransPhase::debugPoint(string msg) {
    if(msg.empty()) {
        cout<< "Debug Point("<<_breakPointNumber<<")"<<endl;
    } else {
        cout<<"MGS: "<<msg <<" on debug Point("<<_breakPointNumber<<")"<<endl;
    }
    cin.get();
    _breakPointNumber++;
}
...
void TransPhase::divideTask() {
    ...
    for(int i =0; i <allSym.size();++i){
        if(allSym[i].is_function()) {
            if(!allSym[i].get_point_of_definition().prettyprint().empty()) {
                //                cout<<"Checking if"<<allSym[i].get_name()<< " has construct inside"<<endl;
    ...
    for(int i =0; i <functionsWithConstructs.size();++i){
        ...
        if(!fD.get_function_body().get_ast().prettyprint().empty()) {
            assignMasterWork(fD.get_function_body().get_ast(), functionsWithConstructs, fD.get_function_symbol().get_name());
        } else {
            FunctionDefinition fD2(functionsWithConstructs[i].get_point_of_definition(),_scope_link);
            if(!fD.get_function_body().get_ast().prettyprint().empty()) {
                assignMasterWork(fD2.get_function_body().get_ast(), functionsWithConstructs, fD.get_function_symbol().get_name());
            }
        }
        ...
        if(expr.is_function_call()) {
            ...
            if(expr_list[l].is_valid() && expr_list[l].get_line()>lastLine 
                ...
                if(test.is_construct()) {
                    ...
                    AST_t loopAst;
                    ScopeLink sL;
                    if(ForStatement::predicate(expr_list[l])) {
                        ForStatement fS(expr_list[l], _scope_link);
                        sL = fS.get_scope_link();
                        loopAst = fS.get_loop_body().get_ast();
                    }
                    
                    if(WhileStatement::predicate(expr_list[l])) {
                        WhileStatement wS(expr_list[l],_scope_link);
                        sL = wS.get_scope_link();
                        loopAst = wS.get_body().get_ast();
                    }
                    if(DoWhileStatement::predicate(expr_list[l])) {
                        DoWhileStatement dWS(expr_list[l],_scope_link);
                        sL = dWS.get_scope_link();
...
string TransPhase::concatenateUnsigned(string s) {
    if(s.find("UNSIGNED") == 0) {
...
void TransPhase::pragma_postorder(PragmaCustomConstruct construct) {
    ...
    if(check_clause.is_defined()) { 
        ...
        _file_tree = construct.get_enclosing_function().get_ast();
        ...
        if(checkDirective(construct,"for")) {
            ...
            //            cout<< std::string(mathExpressiononIteratingVariable) <<endl;
            //            cin.get();
        ...
        if(_isForDirective) {
            ...
            for(int i =0;i<_ioVars.size();++i){
                cout<<std::string(_ioVars[i].name)<<" has "<<_ioVars[i].iterVar.size()<<" possible iterators"<<endl;
            }
            ...
            for(int i =0;i<_inVars.size();++i){
                cout<<std::string(_inVars[i].name)<<" has "<<_inVars[i].iterVar.size()<<" possible iterators"<<endl;
                if(_inVars[i].iterVar.size()>0) {
                    for(int y=0;y<_inVars[i].iterVar.size();++y)
                    cout<<y<<"-"<<std::string(_inVars[i].iterVar[y])<<endl;
                }
            }
                ...
                if(staticC!=0) {
                    ...
                    if(!_withMemoryLimitation) {
                        for(int i = 0; i<_ioVars.size(); ++i){
                            ...
                            upperType = concatenateUnsigned(upperType);
                    ...
                    mpiVariantStructurePart1 << handleMemory(_sourceVar);
                    if(_elseNeeded)
                        mpiVariantStructurePart1 << " else ";
...
string TransPhase::transformConstructAST(PragmaCustomConstruct construct, ScopeLink scopeL, Scope sC, Source initVar){
    ...
    int hasChanges = 0;
    for (ObjectList<AST_t>::iterator it = expr_list.begin();it != expr_list.end(); it++, l++) {
        //debugPoint(expr_list[l].prettyprint());
        ...
        if(test.is_construct()){
            ...
            if(expr_list[l].prettyprint().find_first_of("=")>= 0 && expr_list[l].prettyprint().find_first_of("=")<expr_list[l].prettyprint().length()) {
                ...
                int old_SecureWrite = _secureWrite;
                ...
                if(firstO.find_first_of("[")>= 0 && firstO.find_first_of("[")<firstO.length()) {
                    ...
                    if(finded>-1) {
                        ...
                        if(_fullArrayWrites && std::string(initVar).compare(std::string(firstIterator)) != 0 
                                && lineA>=maxLine && (isIOVar(actArg)|| !_smartUploadDownload) && !_secureWrite) {
                            int rV = 0;
                            for(int r = 0; r<_reducedVars.size();++r) {
                                if(actArg.compare(std::string(_reducedVars[r].name))==0) {
                                    rV = 1;
                                    break;
                                }
                            }
                            ...
                            if(!_secureWrite) {
                                ...
                                if(!f) {
                                    ...
                                    if(itDependent) {
                                        ...
                                        down.start = _ioVars[numVar].size[0];
                                        down.end = "0";
                                    }
                                ...
                                if(_construct_inside_bucle) {
                                   // int exprLine =  construct_ast.get_line();
                                    int exprLine = get_real_line(_file_tree, _scope_link, expr_list[l],0,0,0);
                                    cout<<"altC: "<<exprLine<<endl;
                                    isInRange = checkIfIteratedByOutsideIterators(firstIterator, _construct_loop, scopeL, 1, exprLine);
                                    //TODO send just actual value
                                    _rI = firstIterator;
                                    _rF = firstIterator+" + 1";
                                }
                                if(!isInRange) {
                                    ...
                                    isInRange = isInForIteratedBy(firstIterator, expr_list[l], construct.get_ast(), scopeL, actArg, 1, itList, firstO);
                                }
                        ...
                        if(!knowedVariable) {
                            ...
                            if((_withMemoryLimitation || !isInRange)) {
                                ...
                                //                                cin.get();
                                if(f1 && _ioVars[finded].size.size()>0){
                                    mpiWrites<<"if ("<<firstIterator<<">= "<<_offsetVar<<
                                            " && "<<firstIterator<<" < "<<_partSizeVar<<" + "<<_offsetVar<<" )"
                                ...
                                if(_fullArrayWrites && isInRange) {
                                    ...
                                    if(_downloadVars[numIoVar].start.compare( _ioVars[numVar].size[0]) !=0 && _expandFullArrayReadWrite) {
                                        ...
                                        preWrite << "("<<_coordVectorVar
                                                ...
                                                <<_num_transformed_blocks
                                                <<"[0]));";
                                        _downloadVars[numIoVar].end = std::string(rangeFinishValue);
                                    }
                                    preWrite <<"MPI_Send(&idxForReadWriteSwitch, 1, MPI_INT, 0, "<<_FWTAG<<", MPI_COMM_WORLD);\n "
...
//                                            cin.get();
                            ...
                            if(_fullArrayReads && !_secureWrite) {
                                ...
                                if(isInRange) {
                                    ...
                                    cout<<"read not inRange "<<std::string(operands[e])<<endl;
                                    
                                } else if (isUploadedVar(actArg)) {
                                    cout<<std::string(operands[e])<< " is a Uploaded Var by this range"<<endl;
                            ...
                            if(!knowedVariable) {
                                ...
                                if(!_fullArrayReads || !isInRange) {
                                    ...
                                    if(firstO.find(operands[e])==0) {
//                                        cout<<"OK"<<endl;
                                        mpiReads<<rectifiedVarName<< " = "<<operands[e]<<";\n";
                                    } 
//                                    else {
////                                        cout<<firstO.find(operands[e])<<endl;
//                                    }
//
//                                    cin.get();
...
int TransPhase::isInForIteratedBy(string principalIt, AST_t ast, AST_t astWhereSearch, ScopeLink scopeL, string variableName, int io, ObjectList<string> iteratorOutside, string operatorS) {
    
    TraverseASTFunctor4LocateFor expr_traverseFor(scopeL);
    ObjectList<AST_t> expr_listFor = astWhereSearch.depth_subtrees(expr_traverseFor);
    int lF = 0;
    int minLine = ast.get_line();
    int minLineReal = get_real_line(astWhereSearch, scopeL, ast,1,0,0);
    ...
    for (ObjectList<AST_t>::iterator it = expr_listFor.begin();it != expr_listFor.end(); it++, lF++) { 
        if(ForStatement::predicate(expr_listFor[lF])) {
            ...
            if(inductionVar.compare(principalIt) == 0) {
                ...
                int l = 0;
                int finded = 0;
                int exprLineUse = minLine;
                for (ObjectList<AST_t>::iterator it2 = expr_listUse.begin();it2 != expr_listUse.end(); it2++, l++) { 
                    exprLineUse = expr_listUse[l].get_line();
                    //                    cout<<ast.prettyprint()<<" vs. "<<expr_listUse[l].prettyprint()<<endl;
                    //                    cout<< ast.get_line() <<" vs. "<<exprLineUse<<endl;
                    //                    cout<<"------------------"<<endl;
                    //                    cout<<get_real_line(astWhereSearch.get_enclosing_block(), scopeL, ast,1,0,0)<<endl;
                    //                    cout<<get_real_line(astWhereSearch.get_enclosing_block(), scopeL, expr_listUse[l],1,0,0)<<endl;
                    
                    //                    cin.get();
                    if(ast.prettyprint().compare(expr_listUse[l].prettyprint())==0) {
                        if(ast.get_line() == exprLineUse) {
                            finded = 1;
                            break;
                        }
                        if(get_real_line(astWhereSearch, scopeL, ast,1,0,0) == get_real_line(astWhereSearch, scopeL, expr_listUse[l],1,0,0)) {
                            
                            if(io) {
                                if(ast.prettyprint().find(operatorS)>=0 && ast.prettyprint().find(operatorS)<ast.prettyprint().find("="))   {
                                    finded = 1;
                                    break;
                                }
                            } else {
                                if(ast.prettyprint().find(operatorS)>=ast.prettyprint().find("=") && ast.prettyprint().find(operatorS)<ast.prettyprint().length())   {
                                    finded = 1;
                                    break;
                                }
                            }
                ...
                if(isInForIteratedBy(principalIt, ast, fS.get_loop_body().get_ast(), scopeL, variableName, io, iteratorOutside, operatorS)) {
                    return 1;
                } else {
...
int TransPhase::checkIfIteratedByOutsideIterators(string principalIt, AST_t _construct_loop, ScopeLink scopeL, int io, int exprLine){
    ...
    for (ObjectList<AST_t>::iterator it = expr_listFor.begin();it != expr_listFor.end(); it++, lF++) { 
        if(ForStatement::predicate(expr_listFor[lF])) {
            Statement s(expr_listFor[lF],_scope_link);
            ForStatement fS(s);
            string inductionVar = fS.get_induction_variable().prettyprint();
...
//            int maxL = maxAST.get_line();
            int minL = get_real_line(_file_tree, _scope_link, minAST,0,0,0);
            int maxL = get_real_line(_file_tree, _scope_link, maxAST,0,0,0);
//            cout<<"altMI: "<<minL<<endl;
//            cout<<"altMA: "<<maxL<<endl;
//            get_real_line(,scopeL,minAST)
//                      cout<<inductionVar<<endl;
//                        cout<<"min: "<<minL<<endl;
//                        cout<<"max: "<<maxL<<endl;
...
//                        
//                        cin.get();
            if(inductionVar.compare(principalIt) == 0 && minL < exprLine && maxL >= exprLine && minL>-1 && maxL>-1 && minL<maxL) {
...
string TransPhase::rectifyName(string oldName) {
    ...
    rectifiedVarName = replaceAll(rectifiedVarName,":","Dot");
    rectifiedVarName = replaceAll(rectifiedVarName,"?","Ask");
    rectifiedVarName = replaceAll(rectifiedVarName,"<","Lower");
    rectifiedVarName = replaceAll(rectifiedVarName,">","Greater");
    rectifiedVarName = replaceAll(rectifiedVarName,"=","Equal");
    rectifiedVarName = replaceAll(rectifiedVarName,"&","And");
    rectifiedVarName = replaceAll(rectifiedVarName,"|","Or");
    rectifiedVarName = replaceAll(rectifiedVarName,"!","Not");
...
//                        cout<<", "<<std::string(newR.iterVar[i]);
                for(int j=0;j<iterators.size();++j) {
                    ...
                    if(!find && !iterators[j].empty()) {
                        cout<<"adding "<<std::string(iterators[j])<<" to "<<std::string(newR.name)<<endl;
                        newR.iterVar.push_back(iterators[j]);
//                        cout<<std::string(newR.iterVar[newR.iterVar.size()-1])<<endl;
//                        cin.get();
                    } else {
...
//                        cout<<", "<<std::string(newR.iterVar[i]);
                for(int x=0;x<_ioVars.size();++x) {
                    ...
                    if(std::string(newR.name).compare(std::string(_ioVars[x].name))==0) {
                       // cout<<"adding iterators to "<<std::string(newR.name)<<endl;
...
//                cin.get();
        ...
        if(!findedS.is_valid()){
            ...
            if(findedS.is_valid()){
                ...
                findedS = construct.get_enclosing_statement().get_scope().get_symbol_from_name(newR.name);
                if(findedS.is_valid()){
                    ...
                    cerr<<"Symbol with name ("<<std::string(newR.name)<<" must be at least in function scope"<<endl;
                    exit(-1);
                }
            }
        } else {
        ...
        newR.type <<  declaration;
        cout<<"Size: ";
        for(int si = 0; si <newR.size.size(); si++) {
            cout<<std::string(newR.size[si])<<", ";
        }
        cout<<endl;
        //        cin.get();
        //
...
int TransPhase::isIOVar(string name) {
    ...
    if(_smart_use_table[name].row_first_read_master.row>0) {
        // if((_smart_use_table[name].row_first_write_master.row>=_smart_use_table[name].row_first_read_master.row || _smart_use_table[name].row_first_write_master.row<=0))
        return 1;
    } else if(_construct_inside_bucle) {
        if(_smart_use_table[name].row_last_read_master.for_num == _construct_num_loop)
            if(_smart_use_table[name].row_last_read_master.row<_smart_use_table[name].row_last_write_master.row)
                return 1;
    } else {
    ...
    //    cout<<name<<" discarted as OUT var"<<endl;
    //    cin.get();
    return 0;
}
int TransPhase::isINVar(string name) {
    ...
    //            for(int i = 0; i<_inVars.size();++i) {
    //                if(_inVars[i].size.size()>0) {
    //                    if(std::string(_inVars[i].name).compare(name)==0) {
    //                        newTI.name = name;
    //                        newTI.start = "0";
    //                        newTI.end = _inVars[i].size[0];
    //                        _uploadedVars.push_back(newTI);
    //                        return 0;
    //                    }
    //                }
    //            }
...
AST_t TransPhase::fill_smart_use_table(AST_t asT, ScopeLink scopeL, Scope sC, int outline_num_line, ObjectList<Symbol> prmters , int kind, int offsetLine, AST_t prevAST){
    ...
    for (ObjectList<AST_t>::iterator it = expr_list.begin();it != expr_list.end(); it++, l++) {
        ...
        cout<<"Studied expression("<<l<<"/"<<expr_list.size()<<")("<<line<<","<<outline_num_line<<")"<<endl;
        cout<<"-"<<ppExpr<<"-"<<endl;
        cout<<"____________"<<endl;
...
//                                cin.get();
                            ...
                            if(line<outline_num_line) {
                                if(!kind || kind == 2) {
                                    if((_smart_use_table[actWord].row_last_read_master.row < line || _smart_use_table[actWord].row_last_read_master.row == 0) && isParam(actWord)) {
                                        _smart_use_table[actWord].row_last_read_master = fill_use(line,actAst);
                                    }
                                }  
                            } else {
                                if(!kind || kind == 2) {
                                    if((_smart_use_table[actWord].row_first_read_master.row > line || _smart_use_table[actWord].row_first_read_master.row == 0) && isParam(actWord)) {
                                        _smart_use_table[actWord].row_first_read_master = fill_use(line,actAst);
                                    }
                                } 
                ...
                if(test.is_construct() && f==0 && outline_num_line !=line){
                    cout<<"Detected OpenMP work"<<endl;
                    //                    cin.get();
                    f=1;
//                    if(kind == 2) 
//                        _inside_loop=is_inside_bucle(expr_list[l],scopeL, line, 0);
                    ...
                    if(shared_clause.is_defined()) {
                        ...
                        for (ObjectList<Expression>::iterator it = shared_arguments.begin(); it != shared_arguments.end(); it++) {
                            ...
                            if(!hmpp && kind!=1) {
                                if((_smart_use_table[actWord].row_first_read_master.row>offsetLine || _smart_use_table[actWord].row_first_read_master.row == 0) && isParam(actWord)){
                                    _smart_use_table[actWord].row_first_read_master = fill_use(offsetLine,actAst);
                                }
                                if((_smart_use_table[actWord].row_first_write_master.row>offsetLine || _smart_use_table[actWord].row_first_write_master.row == 0) && isParam(actWord)){
                                    _smart_use_table[actWord].row_first_write_master = fill_use(offsetLine,actAst);
                                ...
                                if(inside) {
                                    if((_smart_use_table[actWord].row_first_write_master.row>offsetLine || _smart_use_table[actWord].row_first_write_master.row == 0) && isParam(actWord)){
                                        _smart_use_table[actWord].row_first_write_master = fill_use(maxLinePragma,actAst);
                                    }
                                }
...
//                        _inside_loop=is_inside_bucle(expr_list[l],scopeL, line, 0);
                ...
                if(!expr.is_function_call() && f == 0) {
                    ...
                    if(expr_listFunc.size()>0) {
                        cout<<"Detected function call by brute force"<<endl;
                        Expression exprUndetected(expr_listFunc[0], _ifScopeL);
                        cout<<exprUndetected.get_called_expression().prettyprint()<<endl;
                        //                        cout<<"Detected function call by brute force"<<endl;
                        //                        if(exprUndetected.get_called_expression().is_named_function_call()) {
                        //                            cout<<"HI"<<endl;
                        //                        }
                        string cE = exprUndetected.get_called_expression().prettyprint();
                        Symbol sym;
                        sym = sC.get_symbol_from_name(cE);
                        if(sym.is_valid()) {
                            if(sym.is_defined()){
                                f = 1;
...
//                        _inside_loop=is_inside_bucle(expr_list[l],scopeL, line, 0);
                    ...
                    if(prmters.size()>0){
                        ...
                        for (ObjectList<Symbol>::iterator it = prmters.begin();it != prmters.end(); it++,k++) {
                            ...
                            //                            pattern2<<"\\s*[^a-zA-Z0-9]*"<<prmters[k].get_name()<<"[^a-zA-Z0-9=]";
                            //                            pattern3<<"[^a-zA-Z0-9]"<<prmters[k].get_name()<<"[^a-zA-Z0-9=]";
                            //                            pattern<<"[^a-zA-Z0-9]"<<prmters[k].get_name()<<"[^a-zA-Z0-9=]";
                            //                            pattern2<<"\\s*[^a-zA-Z0-9]*"<<prmters[k].get_name()<<"[^a-zA-Z0-9=]";
                            //                            pattern3<<"[^a-zA-Z0-9]"<<prmters[k].get_name()<<"[^a-zA-Z0-9=]";
                            pattern<<"\\b"<<prmters[k].get_name()<<"\\b";
                            pattern2<<"\\b"<<prmters[k].get_name()<<"\\b";
                            pattern3<<"\\b"<<prmters[k].get_name()<<"\\b";
                            //                                                        std::cout<<"P: "<<pattern.str()<<endl;
                            //                                                        cin.get();
                            int rv1,rv2,rv3;
                            regex_t exp, exp2, exp3; //Our compiled expression
                            
                            rv1 = regcomp(&exp, pattern.str().c_str(), REG_EXTENDED);
                            if (rv1 != 0) {
                                printf("regcomp failed with %d\n", rv1);
                            }
                            rv2 = regcomp(&exp2, pattern2.str().c_str(), REG_EXTENDED);
                            if (rv2 != 0) {
                                printf("regcomp failed with %d\n", rv2);
                            }
                            rv3 = regcomp(&exp3, pattern3.str().c_str(), REG_EXTENDED);
                            if (rv3 != 0) {
                                printf("regcomp failed with %d\n", rv3);
                            }
                            regmatch_t matches[1]; //A list of the matches in the string (a list of 1)
                            if((regexec(&exp, ppExpr.c_str(), 1, matches, 0) == 0 && !rv1) || 
                                    (regexec(&exp2, ppExpr.c_str(), 1, matches, 0) == 0 && !rv2) || 
                                    (regexec(&exp3, ppExpr.c_str(), 1, matches, 0) == 0 && !rv3))  {    
                                
                                std::string actWord = prmters[k].get_name();
                                Symbol sym = _scope_link.get_scope(_translation_unit).get_symbol_from_name(actWord);
                                int var2Procced = 0;
                                
                                if(sym.is_valid() && inside) {
                                    if(sym.is_defined()){
                                        
                                        var2Procced = 1;
                                        //                                        cout<<"Detected "<<actWord<<" as global variable (this will be allways transfered to ensure proper value updates)"<<endl;
                                        //                                        cin.get();
                                    }
                                } 
                ...
                if(f==0 && ppExpr.substr(0,r.size()).compare(r)==0) {
                    ...
                    if(scopeL.get_scope(expr.get_ast()).get_symbol_from_name(actWord).is_valid()){
                        if(_smart_use_table[actWord].row_first_read_master.row > line || _smart_use_table[actWord].row_first_read_master.row == 0) {
                            _smart_use_table[actWord].row_first_read_master = fill_use(line,actAst);
                        }
                    }
                }
                if(f==0 && ((ppExpr.find("++")>=0 && ppExpr.find("++")<ppExpr.length()) ||
                        (ppExpr.find("--")>=0 && ppExpr.find("--")<ppExpr.length()))) {
                    f=1;
                    ...
                    if(line<outline_num_line) {
                        ...
                        if(!kind || kind==2){
                            if(_smart_use_table[actWord].row_first_write_master.row > line || _smart_use_table[actWord].row_first_write_master.row == 0) {
                                _smart_use_table[actWord].row_first_write_master = fill_use(line,actAst);
                            }
                            if(_smart_use_table[actWord].row_first_read_master.row > line || _smart_use_table[actWord].row_first_read_master.row == 0) {
                                _smart_use_table[actWord].row_first_read_master = fill_use(line,actAst);
    ...
    for (Mymap::iterator itSm = _smart_use_table.begin(); 
        ...
        if(itSm->second.row_first_read_master.row==0) {
            itSm->second.row_first_read_master.ast=asT; 
            itSm->second.row_first_read_master.inside_loop=0; 
            itSm->second.row_first_read_master.for_num=-1; 
            itSm->second.row_first_read_master.for_ast = NULL;
        }
        if(itSm->second.row_first_write_master.row==0) {
            itSm->second.row_first_write_master.ast=asT; 
            itSm->second.row_first_write_master.inside_loop=0; 
            itSm->second.row_first_write_master.for_num=-1; 
            itSm->second.row_first_write_master.for_ast = NULL;
        }
    }
    return lastAst;
}
...
int TransPhase::get_real_line(AST_t asT, ScopeLink scopeL, AST_t actLineAST, int update, int searching_construct, int initialConstruct) {
    ...
    for (ObjectList<AST_t>::iterator it = expr_list.begin();it != expr_list.end(); it++, l++) {
        ...
        if(ppExpr.find(_myidVar+ " == 0")>=0 && ppExpr.find(_myidVar+ " == 0")<ppExpr.length()){
            myidLines++;
        }
        if(update) {
...
Source TransPhase::generateMPIVariableMessagesSend(vector<infoVar> vars, Source initVar, Scope functionScope, string dest, string offset, int withReduced) {
    ...
    if(!_withMemoryLimitation) {
        for(int i = 0; i<vars.size(); ++i){
            ...
            stringstream varSent, size;
            cout<<"-----------"<<endl;
            cout<<std::string(vars[i].name)<<endl;
    ...
    if(_fullArrayReads && _inVars.size()>0) {
        ...
        for (int i = 0; i< _inVars.size();++i){
            ...
            if(_inVars[i].size.size()>0) {
                ...
                memorySource<<"MPI_Recv(&"<<_coordVectorVar<<_num_transformed_blocks<<",2,MPI_INT,"<<source<<", "<<_FRTAG<<", MPI_COMM_WORLD, &"<<_statVar<<");\n";
